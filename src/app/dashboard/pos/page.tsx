            ...
            viewMode === "grid" ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 pb-6">
                {filteredProducts.map((product) => (
                  ...
                  >
                    <div className="mb-4 aspect-square bg-slate-100 border-[2px] border-black flex items-center justify-center">
                      {product.image ? (
              ...
              <div className="flex flex-col gap-4 pb-6">
                {filteredProducts.map((product) => (
                  <div 
                    key={product.id}
                    className={`neo-card flex items-center gap-4 p-4 group cursor-pointer ${product.stock <= 0 ? "opacity-50 grayscale" : "hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"}`}
                    onClick={() => addToCart(product)}
                  >
                    <div className="w-16 h-16 bg-slate-100 border-[2px] border-black flex items-center justify-center shrink-0">
                      {product.image ? (
                        <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-black uppercase truncate">{product.name}</h3>
                      <div className="flex items-center gap-2">
                        <p className="font-bold text-green-600">Rp {product.price.toLocaleString("id-ID")}</p>
                        <span className="text-slate-300">|</span>
                        <span className={`text-xs font-bold ${product.stock < 5 ? "text-red-500" : "text-slate-500"}`}>
                          STOK: {product.stock}
                        </span>
                      </div>
                    </div>
                    <button className="bg-black text-white p-3 border-[2px] border-black shrink-0">
                      <Plus size={20} />
                    </button>
                  </div>
                ))}
